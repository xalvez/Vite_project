...
const About = () => {
  ...
  const observerOptions = {
    threshold: 0.25, // Trigger when 15% of the card is visible
  };
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        // Optional: remove 'active' to animate back out when scrolling away
        // entry.target.classList.remove('active');
      }
    });
  }, observerOptions);

  // Tell the observer to watch every element with the "reveal" class
  ...
  return (
    ...
    >
      <div className="zenith-shade-top-about"></div>
 
      <section className="smooth-section reveal from-bottom delay-1 ">
        <div className="orbit-header">
          <h1 className="orbit-main-title">{translations.title}</h1>
        </div>
        <div className="content-wrapper">
          <div className="about-grid">
            {/* Left Side: Text Section */}
            <div className="about-text-section from-left reveal delay-1">
              {/* Corner Accents */}
            ...
            {/* Right Side: Image Section */}
            <div className="about-image-section from-right reveal delay-1">
              {/* This sensor detects proximity before the mouse hits the image */}
              <div className="proximity-sensor"></div>

              <div className="image-container">
                <div className="main-image">
                  <img src={conceptImage} alt="Bulb" />
                </div>
              </div>
            </div>
          </div>
